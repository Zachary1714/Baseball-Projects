...
predictors <- total %>% 
  ...
  mutate(birthCountry=ifelse(birthCountry=="Australia"|birthCountry=="South Africa"|birthCountry=="Belgium"|birthCountry=="Saudi Arabia"|birthCountry=="Netherlands","Other",birthCountry)) %>%
...
# Assuming you have your training, validation, and test sets

# Scale the training set features
scaled_train_x <- scale(train_x)
scaled_train_y <- scale(train_y)
# Apply the same scaling to the validation set
scaled_validation_x <- scale(validation_x, center = attr(scaled_train_x, "scaled:center"), scale = attr(scaled_train_x, "scaled:scale"))
scaled_validation_y <- scale(validation_y, center = attr(scaled_train_y, "scaled:center"), scale = attr(scaled_train_y, "scaled:scale"))

# Apply the same scaling to the test set
scaled_test_x <- scale(test_x, center = attr(scaled_train_x, "scaled:center"), scale = attr(scaled_train_x, "scaled:scale"))
scaled_test_y <- scale(test_y, center = attr(scaled_train_y, "scaled:center"), scale = attr(scaled_train_y, "scaled:scale"))

nn_model <- keras_model_sequential()

nn_model %>%
  layer_dense(units = 64, activation = 'relu', input_shape = ncol(scaled_train_x)) %>%
...
history <- nn_model %>% fit(
  ...
  epochs = 100,
  batch_size = 32,
  validation_data = list(scaled_validation_x, scaled_validation_y),
  callbacks = list(early_stopping)
)

##This commented out code allows us to check results of the untuned NN which will be irrelevant once we tune the NN
#validation$pred_nn <- (nn_model %>% predict(scaled_validation_x)) * attr(scaled_validation_y, "scaled:scale") + attr(scaled_validation_y, "scaled:center")
#validation$nn_diff <-  validation$remainingWAR - validation$pred_nn
#validation_nn <- validation %>% arrange(nn_diff)


#nn_predictions <- (nn_model %>% predict(scaled_validation_x)) * attr(scaled_validation_y, "scaled:scale") + attr(scaled_validation_y, "scaled:center")
#nn_rmse <- sqrt(mean((nn_predictions - validation_y)^2))
#print(paste("Validation RMSE:", rmse))

#nn_diff <- validation_y - nn_predictions
#nn_diff_sq <- sum(nn_diff^2)

#nn_rsq <- 1-(nn_diff_sq/dev_sq)
#nn_rsq #0.5927666

#MAE_nn <- mean(abs(validation$pred_nn))
#MAE_nn #10.5728

#worst_nn <- validation_nn %>% arrange(nn_diff)%>% dplyr::select(c(ID,remainingWAR,pred_nn,nn_diff,age,WAR,previous_accurred_WAR))
#worst_nn <- head(worst_nn,20)
#worst_nn

```

```{r Tuning the Neural Network, include=FALSE}

# Create a function to build and train the model
build_and_train_model <- function(units, activation, dropout, optimizer, epochs, batch_size) {
  model <- keras_model_sequential() %>%
    layer_dense(units = units, activation = activation, input_shape = ncol(scaled_train_x)) %>%
...
param_grid <- expand.grid(
  units = c(16, 32, 48, 64),
  activation = c('relu'),
  dropout = c(0, 0.2, 0.4, 0.6, 0.8),
  optimizer = c('adam'),
  epochs = c(50, 100, 150, 200),
  batch_size = c(16, 24, 32, 64, 100)
)

# Initialize a list to store trained models
trained_models <- list()

train_fold_indicies <- c(seq(1,length(train_y),length(train_y)/10),length(train_y))
validation_fold_indicies <- c(seq(1,length(validation_y),length(validation_y)/10),length(validation_y))

best_rmse <- Inf
best_model <- NULL
for(i in 1:(length(train_fold_indicies)-1)){
  train_x_fold <- scaled_train_x[train_fold_indicies[i]:train_fold_indicies[i+1],]
  train_y_fold <- scaled_train_y[train_fold_indicies[i]:train_fold_indicies[i+1]]
  
  validation_x_fold <- scaled_validation_x[validation_fold_indicies[i]:validation_fold_indicies[i+1],]
  validation_y_fold <- scaled_validation_y[validation_fold_indicies[i]:validation_fold_indicies[i+1]]
}


# Loop through parameter combinations and train models
for (i in seq_len(nrow(param_grid))) {
  params <- param_grid[i, ]
  model <- build_and_train_model(params$units, params$activation, params$dropout,
                                  params$optimizer, params$epochs, params$batch_size)
  trained_models[[i]] <- model
}

# Print the trained models or further evaluation
#print(trained_models)

# Initialize a vector to store RMSE values
rmse_values <- numeric(length(trained_models))

# Evaluate each trained model on validation data
for (i in seq_along(trained_models)) {
  model <- trained_models[[i]]
  predictions <- model %>% predict(scaled_validation_x)
  rmse_values[i] <- sqrt(mean((predictions - scaled_validation_y)^2))
}
...
best_nn_model <- trained_models[[best_model_index]]
...
#Used for all R-squared calculations as the Sum of Deviance Squared
avg_test <- mean(test_y)
dev_test <- test_y - avg
dev_sq_test <- sum(dev_test^2)
```
...
```{r}
validation$pred_nn <- (best_nn_model %>% predict(scaled_validation_x)) * attr(scaled_validation_y, "scaled:scale") + attr(scaled_validation_y, "scaled:center")
validation$nn_diff <-  validation$remainingWAR - validation$pred_nn
validation_nn <- validation %>% arrange(nn_diff)


nn_predictions <- (best_nn_model %>% predict(scaled_validation_x)) * attr(scaled_validation_y, "scaled:scale") + attr(scaled_validation_y, "scaled:center")
nn_rmse <- sqrt(mean((nn_predictions - validation_y)^2))
print(paste("Validation RMSE:", nn_rmse))
...
nn_rsq <- 1-(nn_diff_sq/dev_sq)
nn_rsq #0.7121073
#If we use this tuned neural network we have the best R-Squared
...
nn_predictions_test <- (best_nn_model %>% predict(scaled_test_x)) * attr(scaled_test_y, "scaled:scale") + attr(scaled_test_y, "scaled:center")
nn_rmse_test <- sqrt(mean((nn_predictions_test - test_y)^2))
print(paste("Test RMSE:", nn_rmse_test))

nn_diff_test <- test_y - nn_predictions_test
nn_diff_sq_test <- sum(nn_diff_test^2)

nn_rsq_test <- 1-(nn_diff_sq_test/dev_sq_test)
nn_rsq_test #0.7121073
#If we use this tuned neural network we have the best R-Squared
...
```
```{r}
# Define the individual model predictions
model_predictions <- cbind(
  validation$pred_xg,
  validation$rF_prediction,
  validation$pred_xg2,
  validation$pred_lasso,
  validation$pred_nn
)

total_rsq <- xg_rsq + rF_rsq + xg_rsq2 + rsq_lasso + nn_rsq

# Define weights for each model (adjust as needed)
model_weights <- c(xg_rsq/total_rsq, rF_rsq/total_rsq, xg_rsq2/total_rsq, rsq_lasso/total_rsq, nn_rsq/total_rsq)

# Calculate the ensemble prediction as a weighted average
validation$pred_ensemble <- rowSums(model_predictions * model_weights)

# Calculate the R-squared value for the ensemble
...
results <- rbind(results,c("Neural Network",NA,nn_rsq,MAE_nn,worst_nn$nn_diff[1]))
...
Let's look at the Shapley values to understand some sense of variable importance
